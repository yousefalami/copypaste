...
https://www.millikitabxana.az/newspapers/search?name=K%C9%99%C5%9Fk%C3%BCl&year=&month=
...
const downloadableLinks = Array.from(elements).map(element => element.getAttribute('source')).filter(source => source && source.endsWith('.pdf'));
